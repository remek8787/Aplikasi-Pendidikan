...
if ($query['token'] == 1) :
    ...
    if ($token == $tokencek['token']) :
        ...
        $query = mysqli_query($koneksi, "SELECT * FROM nilai WHERE id_bank='$idbank' AND id_siswa='$ids' AND id_ujian='$idu'");
        $nilaix = mysqli_fetch_array($query);
        $ceknilai = mysqli_num_rows($query);
        if ($ceknilai <> 0) :
            if ($nilaix['ujian_selesai'] == '') :
                include_once("aturanlanjut.php");
                mysqli_query($koneksi, "UPDATE nilai set online='1' where id_bank='$idbank' AND id_siswa='$ids' AND id_ujian='$idu'");
                jump("?pg=testongoing&idu=$ac&ids=$id_siswa");
            endif;
        else :
