...
if ($pg == 'tambah') {
	$selisih =  strtotime($_POST['sampai']) -  strtotime($_POST['dari']);
	
	$jam   = floor($selisih / (60 * 60));
	$menit = $selisih - ( $jam * (60 * 60) );
	$detik = $selisih % 60;
	
	$ket =  (($jam * 60) + floor( $menit / 60 ))/ $setting['jjm'];
	$kls = fetch($koneksi,'kelas',['kelas'=>$_POST['kelas']]);
	$tingkat = $kls['level'];
	 $where = [
        'kelas'   => $_POST['kelas'],
		'hari'   => $_POST['hari'],
	 ...
	 $cek = rowcount($koneksi, 'jadwal_mengajar', $where);
    if ($cek == 0) {		
	$result = insert($koneksi, 'jadwal_mengajar', $data);
	}
}
if ($pg == 'kelas') {
