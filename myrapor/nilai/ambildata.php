...
if ($pg == 'kelas') {	
     $data = mysqli_query($koneksi, "SELECT kelas FROM kelas where kuri='1'  GROUP BY kelas");           
...
if ($pg == 'kelas2') {	
     $data = mysqli_query($koneksi, "SELECT kelas FROM kelas where kuri='2'  GROUP BY kelas");           
     echo "<option value=''>Pilih Kelas</option>";
     while ($kls = mysqli_fetch_array($data)) {
     echo "<option value='$kls[kelas]'>$kls[kelas]</option>";
    }
...
if ($pg == 'mapelsikap') {
     ...
     $data = mysqli_query($koneksi, "SELECT idmapel,sikap FROM mapel_rapor where sikap='$sikap' GROUP BY idmapel");           
     echo "<option value=''>Pilih Mapel</option>";
     while ($m = mysqli_fetch_array($data)) {
	$pel = fetch($koneksi,'mapel',['id'=>$m['idmapel']]);		
