...
<head>
    ...
    <title>Data Pembayaran Bulan <?= $bulan ?>-<?= $tahun ?></title>
...
<div style='background:#fff; width:97%; margin:0 auto; height:90%;'>
   ...
   <br>
		...
		<center><h4>DATA PEMBAYARAN<br><?= $kode['nama'] ?></h4></center>
		<br>
     ...
     <br>
		 ...
		 <table class='it-grid it-cetak' width='100%'>       
              <tr>
                ...
                <th width="10%" class="text-center">BESAR RP</th>
                <th width="5%" class="text-center">KE</th>
				<th class="text-center">NO BUKTI</th>
            </tr>
                  <?php
				  $no = 0;
			$query = mysqli_query($koneksi,"select * from trx_bayar WHERE blth='$blth' AND kelas='$kelas' AND idbayar='$idb'");
			...
			?>
							...
							<tr>
									...
									<td><?= $siswa['nama'] ?></td>
									<td class="text-right"><?= number_format($data['bayar']) ?></td>
			                        <td class="text-center"><?= $data['ke'] ?></td>
...
$dompdf->render();
$dompdf->stream("Rekap Pembayaran Kelas ".$kelas." Bulan ". $bulane['ket'] . ".pdf", array("Attachment" => false));
exit(0);
?>